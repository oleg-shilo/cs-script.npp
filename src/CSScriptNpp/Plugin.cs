...
namespace CSScriptNpp
{
     ...
     *      + load from history should refresh toolbar (e.g. now debug button is not enabled after loading)
     *      + breakpoint marker should be clickable
     *      + on source position changed ignore event if file does not exist
     *      + On source code position changed/displayed bring the current line into view with scrolling
     *      + if not in break mode all Step* and GO should be ignored
     *      + stop debugging with Shift+F5
     *      + implement Break
     *      + intercept DebugOutput
     *      + process started with F5 cannot be stopped
     ...
     *      - StepIn should not go to no-dbg-symbols assemblie
     *      - handle breakpoints in autogenerated files
     *      - check that F12 works in 'new Test();'
     *      - add/remove breakpoint should trigger 'breakpoint changed' events, which IDE is subscribed to
