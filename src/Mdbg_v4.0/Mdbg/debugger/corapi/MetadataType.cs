...
namespace Microsoft.Samples.Debugging.CorMetadata
{
    public sealed class MetadataType : Type
    {
        ...
        public override PropertyInfo[] GetProperties(BindingFlags bindingAttr)
        {
            ...
            List<PropertyInfo> al = new List<PropertyInfo>();
            //return al.ToArray();
            ...
            try
            {
                ...
                {
                    uint size;
                    m_importer.EnumProperties(ref hEnum, (int)m_typeToken, out fieldToken, 1, out size);
                    if (size == 0)
                        break;
                    Debug.Assert((fieldToken >> 24 == 4) && ((fieldToken & 0x00FFFFFF) >= 1));
                    al.Add(new MetadataPropertyInfo(m_importer, fieldToken, this));
                }
            }
            finally
            {
